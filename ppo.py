...
from craftax.envs.wrappers import LogWrapper, EfficientResetVecEnvWrapper
from models.world_model import FCNetwork
...
def make_train(config):
    ...
    if config["ENV_NAME"] == "Craftax-Classic-Symbolic-v1":
        ...
        env = CraftaxPixelsEnv(CraftaxPixelsEnv.default_static_params())
        is_symbolic = False
    elif config["ENV_NAME"] == "Gridworld-Tech-Tree-v1":
        env = Gridworld()
        is_symbolic = True
    else:
    ...
    env = LogWrapper(env)
    env = EfficientResetVecEnvWrapper(
        env, num_envs=config["NUM_ENVS"], reset_ratio=min(16, config["NUM_ENVS"])
    )
    ...
    def train(rng):
        ...
        if is_symbolic:
            ...
            #     env.action_space(env_params).n, env.get_map_obs_shape()
            # )
