...
def make_craftax_pixel_renderer(block_pixel_size):
    ...
    def render_craftax_pixels(state):
        ...
        def _add_cow_to_pixels(pixels, cow_index):
            ...
            cow_texture_with_background = (
                cow_texture_with_background
                + cow_texture * textures["cow_texture_alpha"]
            )
        ...
        def _add_arrow_to_pixels(pixels, arrow_index):
            ...
            transposed_arrow_texture_alpha = jnp.transpose(
                arrow_texture_alpha, (1, 0, 2)
            )
        ...
        map_pixels = (
            1 - state.is_sleeping
        ) * map_pixels + state.is_sleeping * sleep_pixels
        ...
        def _render_number(pixels, number, x, y):
            pixels = pixels.at[
                y * block_pixel_size + number_offset - 1 : (y + 1) * block_pixel_size
                - 1,
                x * block_pixel_size + number_offset - 1 : (x + 1) * block_pixel_size
                - 1,
