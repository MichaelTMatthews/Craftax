...
def get_player_damage_vector(state):
    blunt_damages = jnp.array(
        [1, 2, 3, 5, 8],
        dtype=jnp.int32,
    )
    blunt_damage = blunt_damages[state.inventory.sword]
    fire_damage = blunt_damage * (state.sword_enchantment == 1) * 0.5
    ice_damage = blunt_damage * (state.sword_enchantment == 2) * 0.5
    ...
    return jnp.stack([blunt_damage, fire_damage, ice_damage], axis=0)
