...
class BlockType(Enum):
    ...
    FIRE_TREE = 31
    ICE_SHRUB = 32
    ENCHANTMENT_TABLE_FIRE = 33
    ENCHANTMENT_TABLE_ICE = 34
    LADDER_DOWN_BLOCKED = 35
    NECROMANCER = 36
    GRAVE = 37
    GRAVE2 = 38
    GRAVE3 = 39
    NECROMANCER_VULNERABLE = 40
...
MOB_TYPE_DAMAGE_MAPPING = jnp.array(
    [
        ...
        [NO_DAMAGE, [6, 0, 0], NO_DAMAGE, [4, 0, 0]],  # knight, arrow2
        [NO_DAMAGE, [6, 2, 2], NO_DAMAGE, [5, 2, 2]],  # troll, slimeball
        [NO_DAMAGE, [2, 4, 0], NO_DAMAGE, [2, 4, 0]],  # pigman, fireball2
        [NO_DAMAGE, [2, 0, 5], NO_DAMAGE, [2, 0, 4]],  # ice troll, iceball2
    ],
    dtype=jnp.float32,
)
...
MOB_TYPE_HEALTH_MAPPING = jnp.array(
    [
        ...
        jnp.array([0, 20, 6, 0]),  # Floor 5 (troll mines)
...
MOB_TYPE_DEFENSE_MAPPING = jnp.array(
    [
        ...
        jnp.array(
            [NO_DEFENSE, [0.2, 0.1, 0.1], [0.0, 0.0, 0.3], NO_DEFENSE]
        ),  # Floor 5 (troll mines)
        jnp.array(
            [NO_DEFENSE, [0.7, 1.0, 0.0], [0.7, 1.0, 0.0], NO_DEFENSE]
        ),  # Floor 6 (fire)
        jnp.array(
            [NO_DEFENSE, [0.7, 0.0, 1.0], [0.7, 0.0, 1.0], NO_DEFENSE]
        ),  # Floor 7 (ice)
...
def load_texture(filename, block_pixel_size):
    filename = os.path.join("craftax/assets", filename)
