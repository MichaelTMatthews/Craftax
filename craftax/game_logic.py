...
def place_block(state, action, static_params):
    ...
    is_placing_crafting_table = jnp.logical_and(
        ...
        jnp.logical_and(
            jnp.logical_not(is_in_solid_block(state, placing_block_position)), has_wood
        ),
    )
    is_placing_crafting_table = jnp.logical_and(
    ...
    is_placing_furnace = jnp.logical_and(
        furnace_key_down,
        jnp.logical_and(
            jnp.logical_not(is_in_solid_block(state, placing_block_position)), has_stone
        ),
    )
    is_placing_furnace = jnp.logical_and(
        is_placing_furnace,
    ...
    is_placing_on_valid_block = jnp.logical_or(
        ...
        == BlockType.WATER.value,
        jnp.logical_not(is_in_solid_block(state, placing_block_position)),
    )
    ...
    is_placing_on_valid_block = jnp.logical_or(
        is_block_grass,
        jnp.logical_or(
            is_block_sand, jnp.logical_or(is_block_path, is_block_ice_grass)
        ),
    )
    ...
    placed_torch_block = jax.lax.select(
        is_placing_torch,
        BlockType.TORCH_ON_PATH.value,
        new_map[placing_block_position[0], placing_block_position[1]],
    )
...
def update_mobs(rng, state, params, static_params):
    ...
    def _move_player_projectile(rng_and_state, projectile_index):
        ...
        magic_damage_coeff = 1 + 0.25 * (state.player_intelligence - 1)
...
def change_floor(state: EnvState, action, static_params: StaticEnvParams):
    on_down_ladder = (
        state.map[
...
def enchant(rng, state: EnvState, action):
    ...
    is_enchanting = jnp.logical_or(is_enchanting_sword, is_enchanting_armour)
...
def craftax_step(rng, state, action, params, static_params):
    ...
    # Change floor
    state = change_floor(state, action, static_params)
    ...
    # Cap inv
    state = clip_inventory_and_intrinsics(state)

    # Inventory achievements
    state = calculate_inventory_achievements(state)

    # Reward
    achievement_coefficients = (jnp.arange(len(Achievement)) >= 25) * 2 + 1
