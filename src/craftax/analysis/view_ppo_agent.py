...
from play_craftax import CraftaxRenderer
...
def main(path):
    ...
    if config["ENV_NAME"] == "Craftax-Symbolic-v1":
        ...
        network = ActorCritic(len(Action), {})
    elif config["ENV_NAME"] == "Craftax-Pixels-v1":
        from craftax.envs.craftax_pixels_env import CraftaxEnv

        env = CraftaxEnv(CraftaxEnv.default_static_params())
        network = ActorCriticConv(len(Action), {})
    else:
...
if __name__ == "__main__":
    checkpoint = (
        "/home/mikey/PycharmProjects/Craftax/wandb/run-20240115_160116-hyb2f3hn/files"
    )
